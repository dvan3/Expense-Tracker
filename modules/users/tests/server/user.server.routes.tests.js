...
describe('User CRUD tests', function() {
  ...
  it('should be able to register a new user', function(done) {
    ...
    agent.post('/api/auth/signup')
      ...
      .end(function(signupErr, signupRes) {
        ...
        signupRes.body.roles.should.be.instanceof(Array).and.have.lengthOf(
          1);
  ...
  it('should be able to login successfully and logout successfully',
    function(done) {
  ...
  it('should not be able to retrieve a list of users if not admin',
    function(done) {
      agent.post('/api/auth/signin')
        .send(credentials)
        .expect(200)
        ...
        .end(function(signinErr, signinRes) {
          ...
          agent.get('/api/users')
            ...
            .end(function(usersGetErr, usersGetRes) {
              ...
              usersGetRes.body.should.be.instanceof(Array).and.have
                .lengthOf(1);
  ...
  it('should be able to update a single user details if admin', function(
    done) {
    user.roles = ['user', 'admin'];
        ...
        .end(function(signinErr, signinRes) {
          ...
          agent.put('/api/users/' + user._id)
            ...
            .end(function(userInfoErr, userInfoRes) {
              ...
              userInfoRes.body.should.be.instanceof(Object);
              userInfoRes.body.firstName.should.be.equal(
                'admin_update_first');
              userInfoRes.body.lastName.should.be.equal(
                'admin_update_last');
              userInfoRes.body.roles.should.be.instanceof(Array)
                .and.have.lengthOf(1);
              userInfoRes.body._id.should.be.equal(String(user._id));
  ...
  it('forgot password should return 400 for non-existent username',
    function(done) {
      user.roles = ['user'];
  ...
  it('forgot password should return 400 for no username provided', function(
    done) {
      ...
      user.save(function(err) {
        ...
        agent.post('/api/auth/forgot')
          ...
          .end(function(err, res) {
            ...
            res.body.message.should.equal(
              'It seems like you signed up using your ' + user.provider +
              ' account');
            return done();
          });
      });
  ...
  it('forgot password should return error when using invalid reset token',
    function(done) {
      user.roles = ['user'];
  ...
  it('should be able to change user own password successfully', function(
    done) {
    agent.post('/api/auth/signin')
      .send(credentials)
      .expect(200)
  ...
  it('should not be able to get any user details if not logged in',
    function(done) {
      // Get own user details
      agent.get('/api/users/me')
        .expect(200)
        ...
        .end(function(signinErr, signinRes) {
          ...
          agent.put('/api/users')
            ...
            .end(function(userInfoErr, userInfoRes) {
              ...
              userInfoRes.body.roles.should.be.instanceof(Array)
                .and.have.lengthOf(1);
              userInfoRes.body.roles.indexOf('user').should.equal(
                0);
              userInfoRes.body._id.should.be.equal(String(user._id));
  ...
  it('should not be able to update own user details with existing username',
    function(done) {
  ...
  it('should not be able to update own user details with existing email',
    function(done) {
  ...
  it('should not be able to update own user details if not logged-in',
    function(done) {
      user.roles = ['user'];
