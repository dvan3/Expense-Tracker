...
var validateEnvironmentVariable = function() {
  var environmentFiles = glob.sync('./config/env/' + process.env.NODE_ENV +
    '.js');
  console.log();
...
var initGlobalConfigFolders = function(config, assets) {
  ...
  config.folders.client = getGlobbedPaths(path.join(process.cwd(),
    'modules/*/client/'), process.cwd().replace(new RegExp(/\\/g), '/'));
  config.folders.server = getGlobbedPaths(path.join(process.cwd(),
    'modules/*/server/'), process.cwd().replace(new RegExp(/\\/g), '/'));
};
...
var initGlobalConfig = function() {
  ...
  var environmentAssets = require(path.join(process.cwd(), 'config/assets/',
  ...
  if (process.env.NODE_ENV !== 'test') {
    config = _.merge(config, (fs.existsSync(path.join(process.cwd(),
      'config/env/local.js')) && require(path.join(process.cwd(),
      'config/env/local.js'))) || {});
  }
